...
<div class="px-4 py-6">
    ...
    @if($loans && $loans->count() > 0)
        ...
        <!-- Pending Payment Loans -->
        @if($loans->where('status', 'pending_payment')->count() > 0)
            <div class="mb-6">
                <h2 class="text-lg font-semibold text-primary-900 mb-4">Pending Payment</h2>
                <div class="space-y-4">
                    @foreach($loans->where('status', 'pending_payment') as $loan)
                        <div class="card p-4">
                            <div class="flex gap-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center gap-2 text-xs text-primary-500 mb-2">
                                        <span class="inline-block px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700">
                                            Pending Payment
                                        </span>
                                    </div>
                                    @php
                                        $payment = $loan->payments()->where('status', 'pending')->first();
                                    @endphp
                                    @if($payment)
                                        <form action="{{ route('payments.checkStatus', $payment->id) }}" method="POST" class="mt-2">
                                            @csrf
                                            <button type="submit" class="text-sm text-primary-600 underline">
        ...
        <!-- Loan History -->
        @if($loans->where('status', '!=', 'active')->where('status', '!=', 'pending_payment')->count() > 0)
            <div>
                <h2 class="text-lg font-semibold text-primary-900 mb-4">Loan History</h2>
                <div class="space-y-4">
                    @foreach($loans->where('status', '!=', 'active')->where('status', '!=', 'pending_payment') as $loan)
                        <div class="card p-4">
                            <div class="flex gap-4">
                                <div class="w-16 h-24 flex-shrink-0 rounded-lg overflow-hidden bg-primary-100">
                                    @if($loan->book->cover_url)
